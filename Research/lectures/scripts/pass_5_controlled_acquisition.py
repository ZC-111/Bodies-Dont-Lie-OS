...
AUTHORIZE_STATES = {"AUTHORIZE", "AUTHORIZE_ACTION", "AUTHORIZED", "authorized"}
DEFER_STATES = {"DEFER", "DEFERRED", "deferred"}
DENY_STATES = {"DENY", "DENY_ACTION", "DENIED", "denied"}
PENDING_STATES = {"PENDING", "pending", "NEEDS_HUMAN_SCOPING", ""}
...
def classify_auth(state: str, decision: str) -> str:
    ...
    if "AUTHORIZED" in upper or "AUTHORIZE" in upper or "AUTHORIZE_ACTION" in upper:
        has_auth = True
    if "DEFERRED" in upper or "DEFER" in upper:
        has_defer = True
    if "DENIED" in upper or "DENY" in upper or "DENY_ACTION" in upper:
        has_deny = True
    if "PENDING" in upper or "NEEDS_HUMAN_SCOPING" in upper:
        has_pending = True
...
def write_preflight_md(
    ...
    shared_units = [u for u in units if u["shared_representation"] == "true"]
    already = sum(
        1 for u in units if str(u.get("local_presence_status", "")).startswith("already_preserved")
    )
    ...
    checks = [
        (
            ...
            all(
                any(e["_auth"] == "AUTHORIZE" for e in u["_ercs"]) for u in units
            )
            if units
