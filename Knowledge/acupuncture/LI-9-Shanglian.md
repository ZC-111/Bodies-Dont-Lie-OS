# LI-9 Shanglian (上廉) — Upper Angle

| Field | Value |
|-------|-------|
| Domain | Acupuncture · Large Intestine channel |
| Code | LI-9 |
| Pinyin | shàng lián |
| Chinese | 上廉 |
| Point class | — |
| Evidence Level | derived — source-verified (Deadman pp. 110–111); cross-source: Bencaodian |
| Last Verified | 2026-09-04 |

## Location

On the radial side of the forearm, 3 cun distal to Quchi LI-11, on the line connecting Quchi LI-11 with Yangxi LI-5.

## Needling

Perpendicular or oblique insertion 0.5 to 1.5 cun.

## Actions

- Harmonises the Large Intestine
- Activates the channel and alleviates pain

## Indications

Borborygmus, Large Intestine qi stagnation, abdominal pain, difficult and dark urination; chest pain, dyspnoea, brain wind, headache; pain or numbness of the shoulder, elbow and arm, numbness of the limbs, hemiplegia from windstroke, cold sensation of the bone marrow.

## Sources

- *A Manual of Acupuncture* (Deadman), pp. 110–111 — read directly from source (column-aware)
- Cross-source: Bencaodian (上廉 Shanglian)
- Validation gate: [large-intestine-channel-validation-v0.1.md](../../Research/acupuncture/large-intestine-channel-validation-v0.1.md)

## Related

- Channel index: [Acupuncture Knowledge Branch](README.md)
- Previous: [LI-8 Xialian](LI-8-Xialian.md) · Next: [LI-10 Shousanli](LI-10-Shousanli.md)
